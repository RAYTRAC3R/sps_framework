...
class Pelt:
    sprites_names = {
        "SingleColour": 'single',
        'TwoColour': 'single',
        'Gradsocks': 'gradient-socked',
        'Gradhoof': 'hoof gradient',
        'Rosette': 'rosette',
        'Smoke': 'smoke',
        'Ticked': 'ticked',
        'Speckled': 'speckled',
    ...
    legs = ["Gradsocks", "Gradhoof"]
    pelt_categories = [legs, plain]
    marking_weights = [1, 2]
    ...
    def pattern_color_inheritance(self, parents: tuple=(), gender="female", missing_parent:dict=()):
        ...
        #note to self: remove the line right below if you decide to restore torties
        torbie = False
...
""")
    ...
    def init_mane(self, parents, missing_parent):
        ...
        if not parents:
            mane_texture = choice(random.choices(Pelt.mane_tex_categories, weights=Pelt.mane_weights))
            tail_texture = choice(random.choices(Pelt.tail_tex_categories, weights=Pelt.tail_weights))
            tail_texture = mane_texture + tail_texture
            print(mane_texture)
            print(tail_texture)
            self.mane_style = choice(random.choices(mane_texture))
            self.tail_style = choice(random.choices(tail_texture))
        else:
            for p in parents:
                parentmane = choice([i.pelt.mane_style for i in parents])
                #print(parentmane)
                parent_mane_texture = mane_texturedict[parentmane]
                parenttail = choice([i.pelt.tail_style for i in parents])
                #print(parenttail)
                parent_tail_texture = tail_texturedict[parenttail]
                possible_textures = [ parent_mane_texture, parent_tail_texture]
                print(possible_textures)
                mane_texture = choice(random.choices(possible_textures))
                tail_texture = choice(random.choices(possible_textures))
                print(mane_texture)
                print(tail_texture)
    ...
    def describe_appearance(cat, short=False):
        ...
        if cat.pelt.marking_tint.replace('r__ ', '') in renamed_colors:
            if cat.pelt.marking == "Gradsocks":
                color_name = f"{color_name} with a {renamed_colors[cat.pelt.marking_tint.replace('r__ ', '')]} leg gradient"
            elif cat.pelt.marking == "Gradhoof":
                color_name = f"{color_name} with a {renamed_colors[cat.pelt.marking_tint.replace('r__ ', '')]} hoof gradient"
        else:
            if cat.pelt.marking == "Gradsocks":
                color_name = f"{color_name} with a {cat.pelt.marking_tint.replace('r__ ', '')} leg gradient"
            elif cat.pelt.marking == "Gradhoof":
                color_name = f"{color_name} with a {cat.pelt.marking_tint.replace('r__ ', '')} hoof gradient"
